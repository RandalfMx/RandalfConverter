...
/**
 * Questa classe viene utilizzata per poter scrivere i tag XML convertendo il testo in formato UTF8
...
public class ConvertToURI extends ConvertTextXml
{
   ...
   * 
   * @param testo Testo da convertire
...
//        testo = testo.r	eplaceAll(" ","%20");
      ...
      {
        MagException magException = new MagException("Il testo ["+testo+"] non rispetta la formattazione URI", MagException.WARNING, MagException.URIERROR, sezione);
  ...
  public String toString(String testo)
  {
    try
    {
//    	System.out.println("Prima: "+testo);
      testo = URLDecoder.decode(testo, charsetName);
//    	System.out.println("Dopo: "+testo); 
    }
    catch (UnsupportedEncodingException e)
    {
      MagException magException = new MagException("Problemi nella conversione del testo URI ["+testo+"] in Stringa", MagException.WARNING, MagException.URIERROR, sezione);
      if (iMagException != null)
        iMagException.add(magException);
      else
        System.err.println(magException.toString());
